...
def add(task, time, lead):
    try:
        ...
        with sqlite3.connect(DB_FILE) as db:
            ...
            cursor.execute("SELECT 1 FROM tasks WHERE description = ?", (task_lower,))
            exists = cursor.fetchone()

            if exists:
                print(f"\033[91m✘ Task with description '{task_lower}' already exists. Only tasks with distinct descriptions are allowed")
                return

            cursor.execute('INSERT INTO tasks (description, scheduled_time, lead_time) VALUES (?, ?, ?)', (task_lower, time, lead))
            db.commit()
